#!/usr/bin/env python3
"""
Front Camera Alignment System
===============================

Simplified version that uses only the front camera for alignment.
Perfect for testing and debugging before moving to two-stage alignment.

Features:
- Uses only front camera
...
class FrontCameraAlignment:
    """
    Front camera alignment system for needle positioning.
    
    Uses only the front camera to align yellow needle with blue/green hair lines.
    """
    ...
    def __init__(self, camera_id=0, arduino_port=None):
        ...
        Args:
            camera_id: Camera index for front camera (default: 0)
        ...
        # Feature flag: Set to True ONLY after uploading updated Arduino code that supports relative commands
        # If False, uses absolute commands (P90, T45) instead of relative (P+1, T-1)
        # DISABLED: Relative commands require perfect sync, which is hard to maintain
        # Using absolute commands with proper syncing is more reliable
        self.use_relative_commands = False  # DISABLED - using absolute commands with sync instead
        
        # Target hair color
        self.target_hair_color = 'blue'  # Which hair to align with ('blue' or 'green')
        
        # Load calibrated HSV ranges
        self.colors = self._load_color_ranges('front_camera_adjustments.json')
    ...
    def set_servo_angles(self, pan_angle=None, tilt_angle=None, use_relative=False):
        """
        Set pan/tilt servo angles.
        
        IMPORTANT: Only moves the servos specified (pan_angle and/or tilt_angle).
        If only one is specified, only that servo moves - the other stays at its current position.
        
        Args:
            pan_angle: Pan servo angle (0-180) or None to leave unchanged
            tilt_angle: Tilt servo angle (0-180) or None to leave unchanged
            use_relative: If True, send relative commands (P+1, T-1) instead of absolute (P90, T45)
        """
        if self.arduino_conn is None:
            ...
            if pan_angle is not None and tilt_angle is not None:
                ...
                print(f"   [SIMULATION] Pan={pan_angle:.1f}° (Tilt unchanged)")
            elif tilt_angle is not None:
                print(f"   [SIMULATION] Tilt={tilt_angle:.1f}° (Pan unchanged)")
            return True
        ...
        try:
            ...
            if pan_angle is not None:
                ...
                if use_relative:
                    ...
                    delta = pan_angle - self.current_pan_angle
                    if abs(delta) > 0.1:  # Only move if delta is significant
                        delta_int = int(round(delta))  # Convert to int for Arduino
                        commands.append(f"P{delta_int:+d}\n")  # P+1 or P-1
                        self.current_pan_angle = pan_angle
                else:
                    # Absolute position
            ...
            if tilt_angle is not None:
                ...
                if use_relative:
                    # Calculate relative movement
                    delta = tilt_angle - self.current_tilt_angle
                    if abs(delta) > 0.1:  # Only move if delta is significant
                        delta_int = int(round(delta))  # Convert to int for Arduino
                        commands.append(f"T{delta_int:+d}\n")  # T+1 or T-1
                        self.current_tilt_angle = tilt_angle
                else:
                    # Absolute position
            ...
            # Send commands one at a time and read responses to keep tracking in sync
            for cmd in commands:
                self.arduino_conn.write(cmd.encode())
                time.sleep(0.1)  # Give Arduino time to process and respond
                
                # Read Arduino's response to sync position tracking
                # Arduino responds with "Pan set to: 91°" or "Tilt set to: 91°"
                start_time = time.time()
                while time.time() - start_time < 0.5:  # Wait up to 0.5s for response
                    if self.arduino_conn.in_waiting > 0:
                        try:
                            response = self.arduino_conn.readline().decode('utf-8', errors='ignore').strip()
                            if response:  # Only process non-empty responses
                                # Parse response like "Tilt set to: 91°" or "Pan set to: 90°"
                                if "Pan set to:" in response:
                                    try:
                                        pan_val = int(response.split("Pan set to:")[1].split("°")[0].strip())
                                        self.current_pan_angle = float(pan_val)
                                        print(f"   📡 Arduino confirmed: Pan={pan_val}°")
                                    except:
                                        pass
                                elif "Tilt set to:" in response:
                                    try:
                                        tilt_val = int(response.split("Tilt set to:")[1].split("°")[0].strip())
                                        # CRITICAL: Always sync Python tracking to Arduino's actual position
                                        old_tilt = self.current_tilt_angle
                                        self.current_tilt_angle = float(tilt_val)
                                        if abs(old_tilt - tilt_val) > 2.0:
                                            print(f"   ⚠️  Position drift detected! Syncing: {old_tilt:.1f}° → {tilt_val}°")
                                        else:
                                            print(f"   📡 Arduino confirmed: Tilt={tilt_val}°")
                                    except Exception as e:
                                        print(f"   ⚠️  Error parsing Tilt response: {e}")
                                break  # Got response, exit loop
                        except:
                            pass  # Ignore parsing errors
                    time.sleep(0.01)  # Small delay to avoid busy-waiting
            
            time.sleep(0.05)  # Brief pause after all commands
            return True
        except Exception as e:
    ...
    def handle_joystick_move(self, servo_type, delta):
        ...
        # Only throttle for small movements (joystick), not for typed large movements
        abs_delta = abs(delta)
        if abs_delta <= 1.0:
        ...
        # For larger movements (typed input like 60 degrees), don't throttle
        
        # Get current position from software tracking
        current_pos = self.current_pan_angle if servo_type == 'pan' else self.current_tilt_angle
        
        # Calculate new position - ONLY move the requested servo
        new_pos = current_pos + delta
        new_pos = max(0, min(180, new_pos))
        ...
        # CRITICAL: Only move the requested servo, not both!
        # This ensures that clicking PAN button only moves PAN, not TILT
        ...
        # Move ONLY the requested servo (keep the other servo at its current position)
        # Always use absolute commands for reliability
        print(f"   🔧 Moving {servo_type.upper()} from {current_pos:.1f}° to {new_pos:.1f}° (Δ{delta:+.1f}°)")
        
        # Safety check: if movement is unexpectedly large, warn user
        if abs(delta) > 1.0 and abs_delta <= 1.0:  # User wanted 1 degree but got more
            print(f"   ⚠️  WARNING: Large movement detected! Expected ~1° but got {abs(delta):.1f}°")
            print(f"   💡 Press 'y' to SYNC positions if movements are incorrect")
        
        # ALWAYS use absolute commands (never relative) to prevent position drift
        if servo_type == 'pan':
            # Only move PAN, explicitly keep TILT at current position
            self.set_servo_angles(pan_angle=new_pos, tilt_angle=None, use_relative=False)
        else:
            # Only move TILT, explicitly keep PAN at current position
            self.set_servo_angles(pan_angle=None, tilt_angle=new_pos, use_relative=False)
        
        # After movement, sync Python tracking from Arduino response (handled in set_servo_angles)
        
        # Wait for servo to move and camera to update
        # Use shorter delay for small movements, longer for large movements
        abs_delta = abs(delta)
        if abs_delta <= 1.0:
            wait_time = 0.2  # Fast for 1-degree movements
        elif abs_delta <= 10.0:
            wait_time = 0.3  # Medium for small movements
        else:
            wait_time = 0.5  # Longer for large movements (60 degrees, etc.)
    ...
    def start_alignment(self):
        ...
        print("\n" + "="*70)
        print("🚀 STARTING ALIGNMENT (Front Camera)")
        print("="*70)
        ...
        # Run calibration if not done (ONLY in AUTO mode, not in manual mode)
        # Calibration moves servos, so we don't want it in manual mode
        if not self.calibration_done:
            print("\n⚠️  Calibration not done. Press 'k' to calibrate manually, or it will run automatically in AUTO mode.")
            # Don't auto-run calibration here - let user control it
            # self.calibrate_servos()  # Disabled - user must press 'k' to calibrate
    ...
    def run(self):
        ...
        print("\n" + "="*70)
        print("🎯 FRONT CAMERA ALIGNMENT SYSTEM")
        print("="*70)
        print("\n📋 Controls:")
        ...
        print("   'y' - SYNC: Set current servo positions (to match physical servos)")
        ...
        while True:
            ...
            window_name = "Front Camera Alignment + Angle Log"
            ...
            if key == ord('s'):
                ...
                # SYNC: Set current servo positions to match physical servos
                print("\n🔄 SYNC: Setting current servo positions")
                print("   This tells BOTH software AND Arduino where the servos actually are")
                print("   This prevents large movements on the first button press")
                try:
                    pan_input = input("   Enter current PAN position (0-180, or Enter to keep current): ").strip()
                    pan_pos = None
                    if pan_input:
                        pan_pos = float(pan_input)
                        pan_pos = max(0, min(180, pan_pos))
                        self.current_pan_angle = pan_pos
                        print(f"   ✅ PAN position synced to {pan_pos:.1f}°")
                    else:
                        pan_pos = self.current_pan_angle
                        print(f"   📍 PAN position unchanged: {pan_pos:.1f}°")
                    
                    tilt_input = input("   Enter current TILT position (0-180, or Enter to keep current): ").strip()
                    tilt_pos = None
                    if tilt_input:
                        tilt_pos = float(tilt_input)
                        tilt_pos = max(0, min(180, tilt_pos))
                        self.current_tilt_angle = tilt_pos
                        print(f"   ✅ TILT position synced to {tilt_pos:.1f}°")
                    else:
                        tilt_pos = self.current_tilt_angle
                        print(f"   📍 TILT position unchanged: {tilt_pos:.1f}°")
                    
                    # Send SYNC command to Arduino to update its internal position tracking
                    if self.arduino_conn:
                        sync_cmd = f"SYNC P{int(pan_pos)} T{int(tilt_pos)}\n"
                        try:
                            self.arduino_conn.write(sync_cmd.encode())
                            time.sleep(0.1)
                            print(f"   ✅ Arduino internal positions synced: Pan={int(pan_pos)}°, Tilt={int(tilt_pos)}°")
                        except Exception as e:
                            print(f"   ⚠️  Could not sync Arduino: {e}")
                    
                    print(f"\n   📊 Current positions: Pan={self.current_pan_angle:.1f}°, Tilt={self.current_tilt_angle:.1f}°")
                    print("   ✅ Next button press will move exactly 1° from these positions")
                except (ValueError, EOFError, KeyboardInterrupt):
                    print("\n   ❌ Sync cancelled or invalid input")
            elif key == ord('k'):
                # Re-calibrate servos
...
def main():
    ...
    print("🎯 FRONT CAMERA ALIGNMENT SYSTEM")
    print("="*70)
    print("\n📖 Simplified version using only front camera")
    print("   Perfect for testing before two-stage alignment")
    print("="*70)
    
    system = FrontCameraAlignment(
