#!/usr/bin/env python3
"""
Front Camera Slider Viewer
Color calibration with sliders for front camera (Camera 0) - yellow separate
"""
...
class FrontCameraSliderViewer:
    ...
    def initialize_camera(self):
        ...
        print(f"✅ Front Camera {self.camera_id} initialized")
        return True
    ...
    def create_hsv_sliders(self):
        """Create HSV adjustment sliders for front camera - yellow separate"""
    ...
    def save_adjustments(self):
        """Save adjustment data to JSON file"""
        final_colors = {}
        ...
        filename = f"front_camera_adjustments.json"
        with open(filename, 'w') as f:
            json.dump(data, f, indent=2)
        print(f"💾 Saved {len(self.adjustment_data)} adjustments to {filename}")
    
    def display_front_camera_with_sliders(self, duration=300):
        """Display front camera with HSV sliders for color calibration"""
        if not self.cap:
            print("❌ Camera not initialized")
            return
        
        print("🎨 Front Camera Color Calibration")
        print("=" * 60)
        print("🎯 Features:")
        print("   🔍 HSV sliders for color calibration")
        print("   📐 Front camera with detected lines")
        print("   🔍 Stable angles (no shaking)")
        print("   📐 2x zoom, 400x400 frame")
        print("   📝 Yellow sliders separate (top)")
        print("   📝 Green/Blue sliders together (bottom)")
        print("Press 'q' to quit, 's' to save frames, 'r' to save adjustments")
        ...
        while time.time() - start_time < duration:
            ...
            if processed is not None:
                # Update HSV ranges from sliders
                ...
                # Add labels
                cv2.putText(result, "Front Camera - Color Calibration", (10, 25), 
                ...
                # Display frame
                cv2.imshow('Front Camera - Color Calibration (2x zoom, 400x400)', result)
                ...
                if key == ord('q'):
                    ...
                    cv2.imwrite(f"front_camera_calibration_{frame_count}.jpg", result)
        ...
        self.save_adjustments()
        print(f"✅ Front camera calibration completed: {frame_count} frames processed")
...
def main():
    ...
    # Initialize viewer
    viewer = FrontCameraSliderViewer(camera_id=0)
    
    if viewer.initialize_camera():
        print("✅ Front camera initialized")
        print("\n📋 Instructions:")
        print("1. Adjust HSV sliders to optimize color detection")
        print("2. Yellow sliders are separate (top window)")
        print("3. Green/Blue sliders are together (bottom window)")
        print("4. Press 's' to save test images")
        print("5. Press 'r' to save adjustment data")
        print("6. Press 'q' to quit")
        
        viewer.create_hsv_sliders()
        viewer.display_front_camera_with_sliders(duration=300)  # 5 minutes
        viewer.release_camera()
    else:
        print("❌ Failed to initialize front camera")
    
    print("✅ Front camera calibration completed!")
